...
Every request the server makes is bounded by one deadline, **10 000 ms** by default. It is the server that gives up, not Jenkins — so a slow instance surfaces as `Jenkins request timed out` even though nothing was refused.

```bash
export MCP_JENKINS_TIMEOUT_MS="30000"
```
...
The value is a positive whole number of milliseconds; anything else is rejected at startup rather than silently falling back. Unlike the credentials below, **one deadline applies to every configured instance** — it is not comma-separated per instance.
