...
```bash
time curl -u username:token https://your-jenkins.com/job/my-job/api/json
```

If that sits comfortably over 10 seconds, raise the deadline past it:

```bash
export MCP_JENKINS_TIMEOUT_MS="30000"
```

See [Configuration](/projects/mcp-jenkins/docs/configuration) for the CLI equivalent.
